...
export default function QuickLogPage() {
  ...
  const handleSave = async () => {
    ...
    if (!household) {
      ...
      showToast("Set up your household first", "error");
      return;
    }

    // Show loading toast
    showToast("Saving transaction...", "loading");
  ...
  return (
    <>
      {/* Toast Notification - OUTSIDE padded container */}
      <div
        className="fixed top-6 left-1/2 z-[9999] transition-all duration-500 ease-out px-6 w-full max-w-md"
        style={{
      ...
      >
        <div
          className="px-5 py-3.5 rounded-2xl border flex items-center gap-3 shadow-2xl"
          style={{
            ...
            boxShadow: toast.type === "success"
              ? "0 10px 40px rgba(52, 211, 153, 0.2)"
              : toast.type === "error"
              ? "0 10px 40px rgba(255, 75, 75, 0.2)"
        ...
        >
          ...
          {toast.type === "loading" ? (
            ...
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" className="flex-shrink-0">
              ...
              <path d="M8 12l3 3 5-6" stroke="#34D399" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          ) : (
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" className="flex-shrink-0">
              <circle cx="12" cy="12" r="10" fill="#FF4B4B" fillOpacity="0.2" />
              <path d="M15 9l-6 6M9 9l6 6" stroke="#FF4B4B" strokeWidth="2.5" strokeLinecap="round" />
            </svg>
          )}
          ...
          {/* Close button (only for non-loading) */}
          {toast.type !== "loading" && (
            <button
              onClick={hideToast}
              className="w-6 h-6 rounded-lg flex items-center justify-center flex-shrink-0 bg-transparent border-none cursor-pointer transition-all hover:bg-white/10"
              style={{
                color: toast.type === "success" ? "#34D399" : "#FF4B4B"
              }}
            >
